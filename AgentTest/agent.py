...
class ReActAgent:
    ...
    def get_api_key() -> str:
        ...
        if not api_key:
            raise ValueError("未找到 API Key，请在 .env 中设置 SILICONFLOW_API_KEY（或 OPENAI_API_KEY）。")
        return api_key

    @staticmethod
    def get_base_url() -> str:
        """Load base URL from env, defaulting to SiliconFlow OpenAI-compatible endpoint."""
        load_dotenv(dotenv_path=".env", encoding="utf-8-sig")
        return (
