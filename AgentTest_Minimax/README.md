...
1. 模型服务切换为 MiniMax（兼容 OpenAI SDK）。
2. 默认模型设置为 `MiniMax-M2.5`。
...
- `MINIMAX_BASE_URL`：可选，默认 `https://api.minimaxi.com`（国内版）。
  - 国内版：`https://api.minimaxi.com`
...
1. `.env` 是否包含正确的 `MINIMAX_API_KEY`
2. `MINIMAX_BASE_URL` 是否为 `https://api.minimaxi.com`（国内版）或 `https://api.minimax.io`（海外版）
